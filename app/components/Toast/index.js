...
export default function Toast({ show, type, text, duration, onClose }) {
  useEffect(() => {
    if (show && duration !== 0) {
      setTimeout(() => {
        onClose();
  ...
  const getIcon = () => {
    ...
    switch (type) {
      ...
      case 'success':
        baseclassName += ' text-green bg-green25/20';
        return (
          <div className={baseclassName}>
            <CheckCircleIcon className='block w-5 h-5' aria-hidden='true' />
